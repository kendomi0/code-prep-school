...
describe("Animation engine functions and variables", () => {
  ...
  describe("setSpeedButtons()", () => {
    it("enables/disables and sets speed buttons accordingly ", () => {
      ...
      expect([
        testEngine.slowBtn.disabled,
        testEngine.mediumBtn.disabled,
        testEngine.fastestBtn.disabled,
      ]).toEqual([false, false, false]);
      expect(
        [testEngine.slowBtn, testEngine.mediumBtn, testEngine.fastestBtn].map(
  ...
  describe("populateSteps()", () => {
    it("populates steps", () => {
      ...
      let steps = [
        ...
        {
          fn: testEngine.updateTextContent,
          args: [testEngine.currentArr, "[1,2,3]"],
        },
      ];
  ...
  describe("addStep()", () => {
    it("adds step", () => {
      ...
      let expectedSteps = [{ fn: testEngine.setGivenArray, args: [arr] }];
  ...
  describe("hideSpeedButtons()", () => {
    it("hides speed buttons", () => {
      ...
      expect(testEngine.speedBtnSection.classList).toContain(
        "hidden-responsive",
      );
    });
  });
  ...
  describe("editSpeed()", () => {
    it("calls playAnimation() if this.currentSpeed is null", () => {
      const spyPlayAnimation = vi
        .spyOn(testEngine, "playAnimation")
        .mockImplementation(() => {});
    ...
    it("calls changeSpeed() if this.currentSpeed is not null", () => {
      ...
      const spyChangeSpeed = vi
        .spyOn(testEngine, "changeSpeed")
        .mockImplementation(() => {});
      testEngine.editSpeed(testEngine.mediumBtn);
