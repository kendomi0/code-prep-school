...
export class AnimationEngine {
  ...
  resetAndHideExcept(...elements) {
    const elementsToKeep = elements.map(
      (element) => this.animatedElements[element],
    );
    let elementsToReset = Object.values(this.animatedElements).filter(
      (el) => !elementsToKeep.includes(el),
    );
